...
import { SidebarItemType } from "../constants/Types";

const SidebarItem = ({ active, value, title, onClick }: SidebarItemType) => (
  <li
    className={`cursor-pointer py-2 px-4 border-b-2 border-gray-300 flex justify-between items-center ${
      active ? "text-green-500" : "text-black"
    }`}
    onClick={onClick}
  >
    <div
      className={`font-bold p-1 border-2 border-black rounded-full w-6 h-6 flex items-center justify-center ${
...
const VideoCoursePage = () => {
  ...
  function navigateToModule(moduleNumber: number) {
  ...
  return (
    <div className="flex h-screen">
      ...
      {sidebarOpen ? (
        <div
          className="bg-white shadow w-64 space-y-4 py-7 px-5 relative"
          onClick={() => setSidebarOpen(false)}
        >
          <div className="flex justify-between items-center">
            <div className="flex items-center">
              <Image src="/logo.png" width={50} height={50} alt="noe" />
              <h3 className="text-2xl font-semibold text-gray-700 ml-3">
                Modules
              </h3>
            </div>
          </div>
        ...
        <div className="bg-white shadow w-20 flex flex-col items-center justify-between py-3 px-2 space-y-4">
          ...
          {[...Array(totalModules)].map((_, i) => (
            <div
              key={i}
              className={`font-bold p-1 border-2 border-black rounded-full w-6 h-6 flex items-center justify-center cursor-pointer ${
                i + 1 === currentModule
